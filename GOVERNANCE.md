...
* [Templates](#templates)
  ...
  * [Notification of Commit Removal for Inactivity](#notification-of-commit-removal-for-inactivity)
...
Be familiar with the [Development](https://docs.cilium.io/en/stable/contributing/development/#dev-guide) and have [2-Factor Authentication](https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/configuring-two-factor-authentication) enabled on your Github account.
...
Any Maintainer may suggest a request for [CNCF Resources](https://www.cncf.io/services-for-projects/) through the [CNCF Service Desk](https://cncfservicedesk.atlassian.net/servicedesk/customer/portal/1). The Maintainers may also choose to delegate working with the CNCF to non-Maintainer community members.
...
* Commitment to meet the expectations described in the [Expectations for Developers with commit access](#expectations-for-developers-with-commit-access)
...
* When the discussion has converged or a reasonable time has elapsed without discussion developing (e.g. a few business days) the nominator calls for a final decision on the candidate with a followup Slack poll.
...
* If the candidate agrees, access is granted by setting up commit access.
  1. Delete the nomination poll and related discussions to preserve the privacy of any discussions regarding the newly nominated committer. If any discussion may be relevant for subsequent project governance discussion, those remarks may optionally be summarized and re-posted to the channel. The final vote summary does not need to be deleted.
...
* In any case, where access is removed, this fact is published through a Slack message to all existing committers (including the candidate for removal). The candidate is also removed from the CNCF’s list of Cilium maintainers as documented [here](https://github.com/cncf/foundation/blob/main/README.md#other-content).
...
* Start a Slack thread with all committers, including the source of the behavior, describing the behavior and the reason it is detrimental to the project. The message should have the same tone as the private discussion and should generally repeat the same points covered in that discussion. The person whose behavior is being questioned should not be surprised by anything presented in this discussion. Ideally the wider discussion provides more perspective to all participants and the issue is resolved. If not,
* Start a Slack thread with all committers except the source of the detrimental behavior requesting a vote on revocation of commit rights. Cite the discussion among all committers and describe all the reasons why it was not resolved satisfactorily. The Slack message should be carefully written with the knowledge that the reasoning it contains may be published to the larger community to justify the decision.
* Each committer may vote yes, no, or abstain by responding to the Slack poll. A failure to reply is an implicit abstention.
...
Votes are counted within the company association and then broken down proportionally. Example: 7 committers from a company vote, 6 votes yes, 1 vote no.
...
Vote results should be reported based on the vote count, i.e. after applying the [Company Block Vote Limit](#company-block-vote-limit).
